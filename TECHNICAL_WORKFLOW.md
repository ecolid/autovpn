...
## 5. 多节点勾选升级 (Multi-Select Update Flow) [v1.1.5]

这是一个典型的“模拟状态机”交互：

1. **状态缓存**: 当用户点击 `/update` 时，Bot 会在内存中为该消息建立一个 `update_registry`。
2. **模拟多选**: 点击节点按钮时，Bot 仅切换按钮的前缀 `☐` -> `✅`，并不发送实际指令。这保证了高频点击时的零延迟。
3. **批量分发**: 点击“开始升级”后，Bot 循环遍历已选中的节点：
    - 如果是 **Master 节点**：直接执行本地 `bash install.sh --update-bot`。
    - 如果是 **Slave 节点**：通过 Worker 接口 `POST /command` 写入任务队列。
4. **异步生效**: 各节点在下一次心跳周期内领走指令，完成全集群同步。

---

## 6. 无感自更新流程 (OTA Update)
