...
- **Guardian Cluster (Cloudflare-Native Serverless 模式) [v1.0 NEW]**：
  - **中继架构**：基于 Cloudflare Workers + KV 的高可用集群管理，**无需 SSH**，**零端口开放**。
  - **信令同步**：节点周期性同步云端“信令”，完美避开 Telegram 频率限制，保障告警 100% 必达。
  - **交互式管理**：在 TG 发送 `/status` 指令，秒级聚合全集群所有节点的 CPU、内存、Xray 状态视图。
  - **远程控制**：无需登录 VPS，通过机器人即可执行 `/restart`、`/logs`、`/link` 等集群级运维操作。
