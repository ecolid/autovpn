                ...
                if (data.result.startsWith("JIT_PUB:")) {
                    ...
                    if (originalTask) {
                        ...
                        await env.DB.prepare("INSERT INTO commands (target_id, cmd, task_id, status) VALUES (?, ?, ?, 'pending')")
                            .bind(patientId, `JIT_MOUNT:${jitPub}`, data.task_id + 1).run();
                    }
...
async function handleTelegramUpdate(update, env) {
    ...
    if (cbData === "join_cmd") {
        ...
        const clusterToken = await getConfig(env, "CLUSTER_TOKEN") || CLUSTER_TOKEN;
        const cmd = `curl -sL https://raw.githubusercontent.com/ecolid/autovpn/main/install.sh | bash -s -- --silent --cf-worker-url ${url} --cluster-token ${clusterToken}`;
        await sendTelegram(BOT_TOKEN, CHAT_ID, `📋 <b>一键加入指令:</b>\n\n<code>${cmd}</code>`, { inline_keyboard: [[{ text: "🔙 返回", callback_data: "show_security" }]] });
        return new Response("OK");
    }
