    ...
    if (text === "/start" || text === "/menu" || cbData === "show_main") {
        ...
        if (nodes.results && nodes.results.length > 0) {
            for (const node of nodes.results) {
                ...
                nodeCards += `├ 指标：X:${x} N:${n} W:${w} | ${qStr}\n`;
                nodeCards += `├ IP: <code>${node.ip || "0.0.0.0"}</code> | v${node.v}\n`;
...
━━━━━━━━━━━━━━━━━━━━━━`;
    ...
    if (text === "/status" || cbData === "show_status") {
        ...
        for (const s of nodes.results) {
            ...
            // 节点信息作为按钮的一部分
            const nodeInfo = `${st} ${s.hostname || s.id} | ${s.ip} | v${s.v}`;
            btns.push([
                { text: nodeInfo, callback_data: `info_${s.id}` },
                { text: `${s.is_selected ? '❌ 取消' : '✔️ 勾选'}`, callback_data: `chk_${s.id}` },
        ...
        if (selectedCount > 0) {
            res += `📦 <b>当前已勾选 <code>${selectedCount}</code> 台设备</b>\n`;
    ...
    if (cbData === "self_update_worker") {
        ...
        try {
            ...
            if (githubVersion === currentVersion) {
                // 已是最新版本
                const info = `✅ <b>已是最新版本!</b>\n\n当前版本：<code>${currentVersion}</code>\nGitHub 版本：<code>${githubVersion}</code>\n\n💡 无需更新，但可以手动刷新检查`;
                const btns = [
...
━━━━━━━━━━━━━━━━━━━━━━`;
    ...
    if (cbData?.startsWith("info_")) {
        ...
        if (node) {
            ...
            const info = `📋 <b>节点信息：${node.hostname || nodeId}</b>\n\n` +
                `├ 状态: ${node.state === 'online' ? '🟢 在线' : '🔴 离线'}\n` +
                `├ IP: <code>${node.ip}</code>\n` +
                `├ 版本: v${node.v}\n` +
                `├ 服务: X:${x} N:${n} W:${w}\n` +
                `└ 上次更新: ${new Date(node.t * 1000).toLocaleString()}`;
            const btns = [[{ text: "🔙 返回", callback_data: "show_status" }]];
            await sendTelegram(BOT_TOKEN, CHAT_ID, info, { inline_keyboard: btns });
        }
        return new Response("OK");
    }
