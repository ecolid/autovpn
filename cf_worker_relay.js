...
请选择统计维度:`;
        ...
        const btns = [
            ...
            [{ text: "📆 月统计", callback_data: "stats_monthly" }, { text: "🏆 流量排行", callback_data: "stats_ranking" }],
            [{ text: "🔙 返回", callback_data: "show_main" }]
        ];
    ...
    if (cbData === "node_management") {
        const nodes = await env.DB.prepare("SELECT id, hostname, state FROM nodes WHERE id != 'INSTALL_VERIFY' ORDER BY hostname").all();
        ...
        if (nodes.results && nodes.results.length > 0) {
            for (const node of nodes.results) {
                const st = node.state === 'online' ? "🟢" : "🔴";
                btns.push([{ text: `${st} ${node.hostname || node.id}`, callback_data: `node_detail_${node.id}`}]);
            }
            btns.push([
                { text: "✅ 批量重启", callback_data: "batch_restart" }, 
                { text: "❌ 批量删除", callback_data: "batch_delete" }
            ]);
        } else {
            res += `暂无节点\n\n`;
        }
        btns.push([{ text: "🔙 返回", callback_data: "show_main" }]);
    ...
    if (cbData?.startsWith("node_detail_")) {
        const nodeId = cbData.replace("node_detail_", "");
        const node = await env.DB.prepare("SELECT * FROM nodes WHERE id = ?").bind(nodeId).first();
        
        if (!node) {
            await sendTelegram(BOT_TOKEN, CHAT_ID, "❌ 节点不存在", { inline_keyboard: [[{ text: "🔙 返回", callback_data: "node_management" }]] });
            return new Response("OK");
        }
        ...
        let t = { up: 0, down: 0 };
        try { h = JSON.parse(node.health || "{}"); } catch (e) {}
        try { t = JSON.parse(node.traffic_total || "{}"); } catch (e) {}
        
        const upGB = ((t.up || 0) / (1024**3)).toFixed(2);
        const downGB = ((t.down || 0) / (1024**3)).toFixed(2);
        
        const info = `🌩️ <b>节点详情</b>

━━━━━━━━━━━━━━━━━━━━━━
ID: <code>${node.id}</code>
主机名：${node.hostname || "未知"}
状态：${node.state === 'online' ? '🟢 在线' : '🔴 离线'}
IP: <code>${node.ip || "0.0.0.0"}</code>
版本：${node.v || "未知"}

💾 系统负荷：${parseFloat(node.cpu || 0).toFixed(1)}%
📊 流量：🔼 ${upGB}GB | 🔽 ${downGB}GB

🔧 服务状态:
  Xray: ${h.xray === 'OK' ? '🟢' : '🔴'}
  Nginx: ${h.nginx === 'OK' ? '🟢' : '🔴'}
  WARP: ${h.warp === 'OK' ? '🟢' : h.warp === 'SKIP' ? '⚪' : '🔴'}
  Loopback: ${h.loop === 'OK' ? '🟢' : '🔴'}
━━━━━━━━━━━━━━━━━━━━━━`;
        
        const btns = [
            [{ text: "🔄 重启节点", callback_data: `restart_node_${node.id}` }],
            [{ text: "🗑️ 删除节点", callback_data: `delete_node_${node.id}` }],
            [{ text: "🔙 返回", callback_data: "node_management" }]
        ];
        await sendTelegram(BOT_TOKEN, CHAT_ID, info, { inline_keyboard: btns });
        return new Response("OK");
    }
    ...
    // 10. 重启节点
    // ============================================================================
    if (cbData?.startsWith("restart_node_")) {
        const nodeId = cbData.replace("restart_node_", "");
        
        const info = `🔄 正在重启节点 ${nodeId}...\n\n⚠️ 此功能尚未实现，需要添加 SSH 连接逻辑`;
        const btns = [[{ text: "🔙 返回", callback_data: "node_management" }]];
        await sendTelegram(BOT_TOKEN, CHAT_ID, info, { inline_keyboard: btns });
        return new Response("OK");
    }
    ...
    // 11. 删除节点
    // ============================================================================
    if (cbData?.startsWith("delete_node_")) {
        const nodeId = cbData.replace("delete_node_", "");
        
        const info = `🗑️ 确定要删除节点 ${nodeId} 吗？\n\n⚠️ 此操作不可逆!`;
        const btns = [
            [{ text: "✅ 确认删除", callback_data: `confirm_delete_${nodeId}` }],
            [{ text: "❌ 取消", callback_data: "node_management" }]
        ];
        await sendTelegram(BOT_TOKEN, CHAT_ID, info, { inline_keyboard: btns });
        return new Response("OK");
    }
    ...
    // 12. 确认删除节点
    // ============================================================================
    if (cbData?.startsWith("confirm_delete_")) {
        const nodeId = cbData.replace("confirm_delete_", "");
        
        await env.DB.prepare("DELETE FROM nodes WHERE id = ?").bind(nodeId).run();
        
        const info = `✅ 节点 ${nodeId} 已删除`;
        const btns = [[{ text: "🔙 返回", callback_data: "node_management" }]];
        await sendTelegram(BOT_TOKEN, CHAT_ID, info, { inline_keyboard: btns });
        return new Response("OK");
    }
